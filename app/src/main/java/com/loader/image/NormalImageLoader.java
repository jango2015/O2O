...
/**
 * @author Michael 2014/10/13
 *         与ImageLoader的区别是在downloadBitmap中采用普通的下载，未采用IgnitedHttpResponse
 */
public class NormalImageLoader extends DataLoader {

	private static final String TAG = "NormalImageLoader";
	...
	public NormalImageLoader(Context context) {
		super(context);
...
//		imageView.setImageDrawable(asyncDrawable);
	...
	public Bitmap processData(Object data) {
		...
		return ContactsHubUtils.corner(bitmap, mLoaderParams.mRoundPx,
				mLoaderParams.mImageWidth);
	}
	...
	public static File downloadBitmap(Context context, String urlString) {
		...
		final DiskLruCache cache = DiskLruCache.openCache(cacheDir,
				HTTP_CACHE_SIZE);
		...
		if (cache != null) {
			...
			if (cache.containsKey(urlString) && cacheFile.exists()) {
				if (BuildConfig.DEBUG) {
					LogUtil.d(TAG, "downloadBitmap - found in http cache - "
							+ urlString);
				}
			...
			try {
				...
				inputStream = connection.getInputStream(); // 取得字节输入流
