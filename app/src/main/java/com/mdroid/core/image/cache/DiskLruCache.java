...
public class DiskLruCache {
	...
	public static DiskLruCache openCache(File cacheDir,
		...
		if (!cacheDir.exists()) {
			...
			LogUtil.d(TAG, "openCache cacheDir mkdirs - " + success);
		}
		LogUtil.d(TAG, "openCache cacheDir exists - " + cacheDir.exists() + " ,isDirectory - " + cacheDir.isDirectory()
				+ " ,canWrite - " + cacheDir.canWrite() + " ,getUsableSpace - " + Utils.getUsableSpace(cacheDir));
		...
		synchronized (mLinkedHashMap) {
			if (mLinkedHashMap.get(key) == null) {
				try {
				    ...
				    LogUtil.e(TAG, "Error in put: " + e.getMessage());
	...
	private void flushCache() {
		...
		while (count < MAX_REMOVALS
			...
			if (BuildConfig.DEBUG) {
			    LogUtil.d(TAG, "flushCache - Removed cache file, " + eldestFile
						+ ", " + eldestFileSize);
			}
		}
	}
	...
	public static File getDiskCacheDir(Context context, String uniqueName) {
		...
		final String cachePath = (Environment.MEDIA_MOUNTED.equals(Environment
				.getExternalStorageState()) || !Utils.isExternalStorageRemovable()) && externalCacheDir != null ? 
						externalCacheDir.getPath() : cacheDirPath;
						
		LogUtil.d(TAG, "getDiskCacheDir cacheDirPath: " + cacheDirPath);
