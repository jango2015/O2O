 ...
 * @author change
 */
public class GetOrderInfo2PayTask extends AsyncTask<String, Void, Boolean> implements Config.CallBack{
	private static final String TAG = GetOrderInfo2PayTask.class.getSimpleName();
	...
	private TelOrderInfo mOrderInfo = new TelOrderInfo();
	private int mPosition = 0;  // 1-从充值服务主入口进入 2-从充值历史进入 3-从充流量进入
	
	public GetOrderInfo2PayTask(final Activity context, Handler handler, int position) {
		mContext = context;
		mHandler = handler;
		mPosition = position;
	}
	...
	protected Boolean doInBackground(String... args) {
		...
		mOrderInfo.mobile = args[1];
		mOrderInfo.total_fee = args[2];
		mOrderInfo.mark_price  = args[3];
		// 订单编号
		...
		String subject = mContext.getResources().getString(R.string.putao_charge_subject);
		try {
			subject = String.format(mContext.getResources().getString(R.string.putao_charge_subject_for_alipay), mOrderInfo.mark_price);
		} catch(Exception e) {
			e.printStackTrace();
		}
		...
		if(pay_type == PaymentDesc.ID_ALIPAY){
		    payByAlipay(subject, orderNo, favo_id);
		}else if(pay_type == PaymentDesc.ID_WE_CHAT){
			// 微信支付 有自己的回调页面，因此根据是否有异常来判断是否需要回调到自己的页面
			needCallback = payByWeChat(subject, orderNo, favo_id);
		}
	 ...
	 * @return
	 */
	private boolean payByWeChat(String subject,String orderNo, String favo_id) {
	    String reqOrderUrl = ChargeUtils.createReqOrderUrlByWeChat(subject, mOrderInfo.product_id, 
                mOrderInfo.mobile, mOrderInfo.total_fee, orderNo, favo_id);
        LogUtil.i(TAG, "reqOrderUrl= "+reqOrderUrl);
        
        boolean needCallback = false;
        try {
            TelOrderInfo orderInfo = ChargeUtils.getChargeOrderInfo(reqOrderUrl, mOrderInfo);
	...
	private boolean payByAlipay(String subject, String orderNo, String favo_id) {
	    String reqOrderUrl = ChargeUtils.createReqOrderUrl(subject, mOrderInfo.product_id, 
                mOrderInfo.mobile, mOrderInfo.total_fee, orderNo, favo_id);
        LogUtil.i(TAG, "reqOrderUrl= " + reqOrderUrl);
        ...
        try {
             ...
             */
            TelOrderInfo orderInfo = ChargeUtils.getChargeOrderInfo(reqOrderUrl, mOrderInfo);
            alipayReqOrderUrl = ChargeUtils.getAlipayOrderInfo(orderInfo);
            //modified by hyl 2014-10-13 end
	...
	private int parseAlipayResult() {
		...
		int errcode = 0;
		TelOrderInfo orderRsp = mOrderInfo;
		try {
