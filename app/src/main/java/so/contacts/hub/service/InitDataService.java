...
public class InitDataService extends IntentService implements LBSServiceListener, TagAliasCallback{
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        mMainHandler.sendEmptyMessageDelayed(MSG_LOAD_MOVIE_CITY,20*60*1000);
    }
    ...
    private static final int MSG_LOAD_MOVIE_CITY = 1002;
    public static final int MSG_REPORT = 1003;

    private static final int LOAD_MOVIE_CITY_INTERVAL_TIME = 24*60*1000;// 24小时间隔
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_SET_ALIAS:{//add by hyl 2014-12-23 设置别名和标签
                    PushUtil.setAliasTags(ContactsApp.getContext(), InitDataService.this);
                    break;
                }
                ...
                case MSG_REPORT:
                {
                    ...
                    //有未上报数据,则启动上报
                    if(NetUtil.isNetworkAvailable(InitDataService.this)) {
                        if(MsgReportUtils.getUnReportedMsg(InitDataService.this) > 0) {
                            if(mMsgReportThread == null || !mMsgReportThread.isAlive() || mMsgReportThread.finished()) {
                                mMsgReportThread = new MsgReportThread(InitDataService.this, mMainHandler);
    ...
    public class PlugServiceImpl extends IPutaoService.Stub {
        ...
        public boolean uploadMark(String number, String mark) throws RemoteException {
            ...
            MobclickAgentUtil.onEvent(InitDataService.this, UMengEventIds.DISCOVER_YELLOWPAGE_NUMBER_MARK);
            MobclickAgentUtil.onEvent(InitDataService.this,
...
//            com.putao.analytics.MobclickAgentUtil.onPageStart("yellowLive"); //统计页面
            ...
            MobclickAgentUtil.onResume(InitDataService.this);//add by hyl 2014-9-22
...
//            com.putao.analytics.MobclickAgentUtil.onPageEnd("yellowLive"); //统计页面
            ...
            MobclickAgentUtil.onPause(InitDataService.this);//add by hyl 2014-9-22
        }
...
//		}
        ...
        public void addUMengEvent(String umengEventId, int time) throws RemoteException {
            ...
            if( !TextUtils.isEmpty(umengEventId) ){
                if (time <= -1) {
                    MobclickAgentUtil.onEvent(InitDataService.this, umengEventId);
                } else {
                    HashMap<String, String> data = new HashMap<String, String>();
                    data.put("__ct__", String.valueOf(time));
                    LogUtil.d(TAG, "um plug time enter: " + time);
                    MobclickAgentUtil.onEvent(InitDataService.this, umengEventId, data);
                }
            }
        }
...
//    }
    ...
    private void initData() {
        new Thread(new Runnable() {
            @Override
            public void run() {
                EventUtil.markReport(InitDataService.this,false);//add by hyl 2014-10-25 每次重新启动 都将日志标识为未上报

                //add ljq 2014_11_20 start 将耗时初始化动作从ContactsApp放在这里来做  特别是数据库的初始化和升级
                SDKInitializer.initialize(InitDataService.this.getApplicationContext());
                if(!HmtSdkManager.getInstance().isInit()) {
                    HmtSdkManager.getInstance().init(InitDataService.this.getApplicationContext());
                }
