...
public class YellowPageCtripActivity extends YellowPageH5Activity{
	private static final String TAG = YellowPageCtripActivity.class.getSimpleName();
	...
	protected void configWebSettings() {
		...
		CookieSyncManager.createInstance(YellowPageCtripActivity.this);		
	...
	public class MyPutaoWebClientProxy extends PutaoWebClientProxy {
		...
		public void putao_onReceivedTitle(WebView view, String title) {
			...
			if(!TextUtils.isEmpty(view.getUrl()) && view.getUrl().equals(mUrl) && mPageFinished && progress==100) {
				LogUtil.d(TAG, "close progress.");		
				mHandler.sendEmptyMessage(MSG_DISMISS_DIALOG_ACTION);
			}
		}
	...
	private String onloadJs_def() {
		String jscode = 		
			"try { "+
					...
					// 首页aside标签的css cont_wrap高度固定了
					"var aside = document.getElementsByTagName('aside');"+ 
					"if(aside && aside[0]) {"+
					"    aside[0].className='mt10';"+
					"}"+
					
				    // 查询页menu标签的css cont_wrap高度固定了
				    "var menu = document.getElementsByTagName('menu');"+ 
					"if(menu && menu[0]) {"+
					"    menu[0].className='mt10';"+
					"}"+
					
					// 选择城市列表citylistroot标签的css citylistroot高度固定了
					"var citylistroot = document.getElementById('citylistroot');"+ 
					"if(citylistroot) {"+
					"    citylistroot.className='mtm1';"+
					"}"+
					
					//选择日期article标签的css cont_wrap高度固定了
//					"var article = document.getElementsByTagName('article');"+ 
//					"if(article && article[0]) {"+
//					"    article[0].className='m10';"+
//					"}"+

					"var flight_zjfooter='flight-zjfooter';"+  // 首页、查询页尾巴
					"var flight_header_blank='flight-header-blank';"+    // 详情页尾巴
					"var allElements = document.getElementsByTagName('div');"+
					"var count=0;"+
					"for(var i=allElements.length-1; i>=0; i--)"+
					  "{ "+						
							"if (allElements[i].className == flight_zjfooter) {"+
...
//	                    UMengEventIds.DISCOVER_YELLOWPAGE_RESUME_TIME_54, map_value, time);
