...
import so.contacts.hub.ui.yellowpage.bean.ChargeTelephoneMessageBusiness.ChargeOrderStatus;

import so.contacts.hub.core.ConstantsParameter;

import so.contacts.hub.remind.BaseRemindActivity;

import so.contacts.hub.ui.yellowpage.bean.TelephoneOrderBean;
...
public class YellowPageChargeTelephoneOrderActivity extends BaseRemindActivity implements OnClickListener,IResponse,RefreshOrderListener,PaymentCallback{
    ...
    private TelephoneOrderBean order;
    ...
    private void refreshUI(){
        ...
        if (null == orderBean) {
            if(isNeedLoadData){
                ...
                Toast.makeText(YellowPageChargeTelephoneOrderActivity.this,
        ...
        if(!TextUtils.isEmpty(orderBean.getExpand())){
            order =  new Gson().fromJson(orderBean.getExpand(), TelephoneOrderBean.class);
        ...
        if(order != null){
            ...
            orderTitle.setText(R.string.putao_charge_subject);
            orderContent.setText(order.getAttribution());
            orderPhoneNum.setText(order.getMobilenum());
            orderStatus.setText(orderBean.getStatus());
            if (ChargeTelephoneMessageBusiness.isOrderOutDate(orderBean)) {
            ...
            orderProductDes.setText(getString(R.string.putao_charge_telephone_order_content_detail, order.getAttribution(),order.getFace_value()));
            orderPhone.setText(order.getMobilenum());
            if(!TextUtils.isEmpty(order.getAttribution())){
                if(order.getAttribution().contains("电信")){
                    operatorLogo.setImageResource(R.drawable.icon_btn_id_huafei_a);
                }else if(order.getAttribution().contains("移动")){
    ...
    private void payOrder(){
        ...
        param.putSubObj("face_value", order.getFace_value());
        param.putSubObj("attribution",order.getAttribution());
        
        
        param.putUIPair("mobile_ui", order.getMobilenum() + "  " + order.getAttribution());
        param.putUIPair("traffic_value", order.getFace_value());
        
        param.setPriceInCents(orderBean.getPrice());
        String couponIds=orderBean.getCoupon_ids();
        if(!TextUtils.isEmpty(couponIds)){
            String[]coupons=couponIds.split(",");
            if(null!=coupons && coupons.length>0){
                for(int i=0;i<coupons.length;i++){
                    try{
                        long id=Long.valueOf(coupons[i]);
                        param.addCounponId(id);  
                    }catch(Exception e){
                        e.printStackTrace();
                    }
                }
        ...
        addUmengEvent(param,orderBean.getPayment_type());
     ...
     * @param orderParam
     */
    private void addUmengEvent(GetOrderParam orderParam,int payType){
            ...
            case R.id.order_pay_btn:
                ...
                if (isFromMyOrder) {
                    MobclickAgentUtil.onEvent(ContactsApp.getInstance().getApplicationContext(),
                            UMengEventIds.DISCOVER_YELLOWPAGE_MY_ORDER_CENTER_TELEPHONE_IMMEDIATE_PAY);
                }
