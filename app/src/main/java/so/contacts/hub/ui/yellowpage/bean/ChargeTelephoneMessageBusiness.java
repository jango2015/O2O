...
public class ChargeTelephoneMessageBusiness extends AbstractMessageBussiness {

    private Context context;

    private final String SHARED_PREFS_KEY_TRAFFIC = "message_Charge_telephone";
    ...
    public ChargeTelephoneMessageBusiness(Context context) {
        super(context);
        super.productType = MsgCenterConfig.Product.charge.getProductType();
        super.logoId = R.drawable.putao_icon_order_chf;
        super.smallLogoId = R.drawable.putao_icon_order_hf_s;
        super.title = R.string.putao_charge_tag_title_charge;
        this.context = context;
        PTOrderCenter.getInstance().register(this);
        umengInsertDataEventId = UMengEventIds.DISCOVER_YELLOWPAGE_MY_MSG_CENTER_TELEPHONE_ITEM_NUM;
    }

    @Override
    public View getOrderView(PTOrderBean bean, View convertView) {
        TelephoneOrderBean order = getTelephoneOrderBeenFromMessage(bean);
        if (order != null) {
            ...
            holder.title.setText(R.string.putao_charge_subject);
            holder.subject.setText(context.getResources().getString(R.string.putao_charge_telephone_order_content, order.getAttribution(),order.getFace_value()));
    ...
    public void click(PTMessageBean bean, Activity context) {
        ...
        MobclickAgentUtil.onEvent(ContactsApp.getInstance().getApplicationContext(),
                UMengEventIds.DISCOVER_YELLOWPAGE_MY_MSG_CENTER_TELEPHONE_ITEM_CLICK);
    ...
    public void click(PTOrderBean bean, Activity context) {
        ...
        MobclickAgentUtil.onEvent(ContactsApp.getInstance().getApplicationContext(),
                UMengEventIds.DISCOVER_YELLOWPAGE_MY_ORDER_CENTER_TELEPHONE_ITEM_CLICK);
        //add xcx 2014-12-31 end 统计埋点
    }

    private void onClick(String orderNo, Activity context,boolean fromMyOrder) {
        Intent intent = new Intent(context, YellowPageChargeTelephoneOrderActivity.class);
    ...
    public void handleBusiness(PTMessageBean message) {
        // modify by xcx 2015-01-14 start 话费支付成功和充值成功的通知不在系统通知栏显示
        if (null == message) {
            return;
        }
        ...
        // modify by xcx 2015-01-14 end 话费支付成功和充值成功的通知不在系统通知栏显示
    }

    @Override
    public boolean isOrderExpire(PTOrderBean order) {
        boolean isOderExpire=false;
    ...
    public View getConfigView(Activity context) {
        ...
        tv.setText(R.string.putao_charge_telephone_setting_hint);
        return view;
    }
     ...
     * @return
     */
    private TelephoneOrderBean getTelephoneOrderBeenFromMessage(PTOrderBean bean) {
        if(null==bean){
            return null;
        }
        String expand_param = bean.getExpand();
        if (!TextUtils.isEmpty(expand_param)) {
            TelephoneOrderBean order = null;
            try {
                Gson gson = new Gson();
                order = gson.fromJson(expand_param, TelephoneOrderBean.class);
                return order;
            } catch (Exception e) {
                e.printStackTrace();
            }
