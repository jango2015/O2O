 ...
 */
package so.contacts.hub.ui.yellowpage.tag;
...
public class TitleIndicator extends LinearLayout implements View.OnClickListener,
    ...
    private static final float FOOTER_LINE_WIDTH = 60;

    private static final int FOOTER_COLOR = 0xFFFFC445;

    private static final float FOOTER_TRIANGLE_HEIGHT = 10;
    
    private static final int ONTOUCH_STYLE = -1;
    
    public static final int ONTOUCH_STYLE_SMALL = 1;
    
    public static final int ONTOUCH_STYLE_TALL = 2;
    ...
    //滚动条的宽度
    private float mFooterLineWidth;
    //点击时的样式
    private int mOnTouchStyle = -1;
    ...
    public TitleIndicator(Context context, AttributeSet attrs) {
        ...
        int footerColor = a.getColor(R.styleable.TitleIndicator_tag_footerColor, FOOTER_COLOR);
        mTextColor = a.getColorStateList(R.styleable.TitleIndicator_tag_textColor);
        mTextSizeNormal = a.getDimension(R.styleable.TitleIndicator_tag_textSizeNormal, 0);
        mTextSizeSelected = a.getDimension(R.styleable.TitleIndicator_tag_textSizeSelected, mTextSizeNormal);
        mFooterLineHeight = a.getDimension(R.styleable.TitleIndicator_tag_footerLineHeight,
                FOOTER_LINE_HEIGHT);
        mFooterTriangleHeight = a.getDimension(R.styleable.TitleIndicator_tag_footerTriangleHeight,
                FOOTER_TRIANGLE_HEIGHT);
        mFooterLineWidth = a.getDimension(R.styleable.TitleIndicator_tag_footerLineWidth,
                FOOTER_LINE_WIDTH);
        mOnTouchStyle = a.getInteger(R.styleable.TitleIndicator_tag_onTouchStyle,
                ONTOUCH_STYLE);
    ...
    protected void onDraw(Canvas canvas) {
         ...
         */
        //modity ljq 2014_12_27 start 可以指定宽度 mFooterLineWidth line宽度
        float width_px =  mFooterLineWidth;
        float offset = (mPerItemWidth - width_px) /2;
        float left_x = mSelectedTab * mPerItemWidth + offset + scroll_x;
        float right_x = (mSelectedTab + 1) * mPerItemWidth - offset + scroll_x;
        //modity ljq 2014_12_27 end 可以指定宽度
//      Log.d(TAG, "move left_x " + left_x + " right_x " + right_x + " offset " + offset);
    ...
    protected void add(int index, String label, int icon, boolean hasTips) {
        View tabIndicator;
        if (index < 2) {
            tabIndicator = mInflater.inflate(R.layout.putao_title_flow_indicator, this, false);
        } else {
            tabIndicator = mInflater.inflate(R.layout.putao_title_flow_indicator_v2, this, false);
        }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            switch (mOnTouchStyle) {
                case ONTOUCH_STYLE_SMALL:
                    v.setBackgroundResource(R.color.putao_gewala_tag_bar_ontouch);
                    break;
                case ONTOUCH_STYLE_TALL:
                    TextView title = ((TextView)v.findViewById(R.id.tab_title));
                    if(title != null){
                        v.setSelected(true);
                        setTabTextSize(v, true);
                    }
                    break;
                default:
                    break;
            ...
            switch (mOnTouchStyle) {
                case ONTOUCH_STYLE_SMALL:
                    v.setBackgroundResource(R.color.putao_transparent);
                    break;
                case ONTOUCH_STYLE_TALL:
