...
public class PinyinHelper {
	...
	static {
		 String[] mulitVoiceList = ContactsApp.getInstance().getResources().getStringArray(R.array.putao_mulit_voice);
		 if(mulitVoiceList != null & mulitVoiceList.length > 0) {
			 for (int i = 0; i < mulitVoiceList.length; i++) {
				 String multiVoice = mulitVoiceList[i];
				 String[] multiVoiceData = multiVoice.split(",");
				 if( multiVoiceData != null && multiVoiceData.length == 2 ){
					 mulitVoiceMap.put(multiVoiceData[0], multiVoiceData[1]);
				 }
			 }
		 }
...
//		mulitVoiceMap.put("重庆", "chong 重 qing 庆");
//		mulitVoiceMap.put("长治", "chang 长 zhi 治");
//		mulitVoiceMap.put("长春", "chang 长 chun 春");
//		mulitVoiceMap.put("长沙", "chang 长 sha 沙");
//		mulitVoiceMap.put("乐山", "le 乐 shan 山");
//		mulitVoiceMap.put("乐东", "le 乐 dong 东");
	}
	...
	private void myReader() {
		...
		String pinyin;
		try {
			InputStream inStream = ContactsApp.getInstance().getAssets().open("putao_resource.txt");
...
//					.getResourceAsStream("resource.txt")));
	...
	public String getFirstLetter(String value) {
		...
		if (arr != null && arr.length > 0) {
			if (arr[0] > 128) {
				unicode = Integer.toHexString(arr[0]);// 得到汉字的unicode编码
				pinyin = hashMap.get(unicode);
				if (pinyin == null) {// 特殊符号
					firstLetter = null;
				} else {
				...
				firstLetter = String.valueOf(arr[0]).toUpperCase();
			}
		}
		return firstLetter;
	}
